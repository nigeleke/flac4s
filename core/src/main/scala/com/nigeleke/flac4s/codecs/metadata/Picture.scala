...
object Picture:
  ...
  //
  private val pictureTypeCodec  = uint32
  private val descriptionCodec  = ascii32
  private val widthCodec        = uint32
  private val heightCodec       = uint32
  private val colourDepthCodec  = uint32
  private val coloursCountCodec = uint32
  private val dataCodec         = uint32.flatZip { size => codecs.bits(size * 8) }
