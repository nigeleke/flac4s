...
object Cuesheet {
  ...
  //
  private val catalogNumberCodec      = log("c:catalogNumber")(fixedSizeBytes(128, ascii))
  private val leadInSamplesCountCodec = log("c:leadInSamples")(bytes(8))
//    bytes(8).xmap(
//      bv => BigInt(+1, bv.toArray),
//      bi => ByteVector(bi.toByteArray.takeRight(8))
//    )
  private val isCdCuesheetCodec = log("c:isCdCuesheet")(bool)
  private val reservedCodec     = log("c:reserved")(constant(BitVector.fill(7 + 258 * 8)(false)))
  private val tracksCodec       = log("c:tracks")(vectorOfN(uint8, CuesheetTrack.codec))

  def codec(using header: MetadataBlock.Header): Codec[Cuesheet] = log("cuesheet") {
    catalogNumberCodec ::
